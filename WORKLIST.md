# WORKLIST.md — Nest work history (FROZEN 2026-05-21)

> **This file is no longer the source of truth for active work.** As of 2026-05-21 active task state lives in Linear (workspace "AI Kanban Pilot", team key `AI`), queried via the `linear` MCP server. The 40 open items that lived here were migrated to tickets `AI-5`…`AI-44`; see `scripts/tasks-migration.yaml` for the audit trail and `reference_linear_mcp.md` memory for the workflow rule.
>
> Existing `[x]` items and the dated log below remain as institutional memory — searchable history of what shipped and why. **Do not append new entries here.** New work goes to Linear; completed work history accrues in Linear's "Done" lane.
>
> Original header (preserved for context):
> Markers: `[x]` done · `[ ]` not started · `[~]` mid-edit · `[/]` partial · `[?]` evidence weak. Gates: `REASSESS`, `HUMAN`.
...
## Step 4.5 — Custom Nest chat interface + multi-engine router  **[CANCELLED 2026-05-21]**

> **Cancelled 2026-05-21.** Hermes was selected and is running; OpenClaw, Hermes, Codex CLI, Claude Code each turned out to be self-contained agent scaffolds that the user picks per task. No Nest-owned router needed; OpenRouter is the escape hatch for any future generic LLM routing. `hub/src/routes/router.js` and `/api/router/recommendation` were deleted. The capacity-aware ledger (C10) remains useful as informational `/tokens` page, just not feeding a router. Linear tickets `AI-5`/`AI-6`/`AI-7`/`AI-8`/`AI-9` were cancelled with evidence comments. Section preserved below for historical context only.
...
- 2026-05-21 — **Source-of-truth shift: Linear is the new tracker.** WORKLIST.md frozen, ROADMAP.md reframed as strategic plan (40 forward-looking markers stripped, 20 `[x]` evidence markers preserved), AGENTS.md `§Nest Development Workflow` rewritten around the kanban states + WIP limits, CLAUDE.md gets a one-line pointer to Linear. Migrated all open items: `linear.yaml` now defines 6 phase Projects (3-trailing/4/5/6/7/8); `apply-linear-config.py` extended with project reconcile + adapted to Linear's 10k query-complexity cap (split into 4 small queries — combined was 11k). `scripts/tasks-migration.yaml` (audit trail, 40 tickets) + `scripts/migrate-tasks-to-linear.py` (idempotent by title) shipped together. First run created `AI-5`…`AI-44` distributed correctly (9/7/7/5/8/4 across the 6 projects). Second migrator run: 40/40 skipped. Each ticket has `needs-spec` + suitability + kind labels; bodies link back to original ROADMAP/WORKLIST source lines. Final WORKLIST entry; new entries go to Linear from here on.
- 2026-05-21 — **Linear MCP + config-as-code shipped.** Adopted `@tacticlaunch/mcp-linear@1.1.2` (stdio, token-auth — official OAuth MCP doesn't survive headless agents). One token in `config.env`, sourced by `scripts/run-linear-mcp.sh`; registered in three slots so every agent type picks it up: `/opt/nest/.mcp.json` (project Claude Code), `~/.claude.json` top-level `mcpServers` (any-cwd Claude Code), and `[mcp_servers.linear]` in both Codex `config.toml`s (`~/.codex/` direct + `~/.openclaw/agents/main/agent/codex-home/` for openclaw-spawned codex). OpenClaw itself has no MCP slot — agents bring their own. Smoke test: MCP `initialize` returned `linear v1.1.2` cleanly. **Config-as-code:** `config/linear.yaml` (6 workflow states + 9 labels + WIP-limit metadata) + `scripts/apply-linear-config.py` (Python stdlib + PyYAML, no new deps); sidecar at `state/linear.json` (gitignored). First run on team `AI` (Florent's "AI Kanban Pilot" workspace): created 4 states + 8 labels, updated 2 of Linear's defaults (Backlog/Done) to match YAML colors. Second run: 6/6 states + 8/8 labels `unchanged` — idempotency proven. Safety contract: additive-only by default; opt-in `--prune` flag (with `--dry-run` + `--yes`) archives empty leftover states and deletes leftover labels, refusing non-empty states. Used `--prune` to clean Linear's onboarding cruft: archived 4 default states (`Todo`/`In Progress`/`In Review`/`Canceled`), deleted 3 default labels (`Bug`/`Feature`/`Improvement`); had to ad-hoc-delete 4 Linear-generated welcome tickets (`AI-1…4`) first so `Todo` could be archived; `Duplicate` (type=`duplicate`) is Linear-reserved and gets silently skipped on future runs. Final board: 6 user states + 1 reserved, 8 labels. Token now exists in the chat transcript — plan to rotate post-pilot.
- 2026-05-21 — **Mail end-to-end confirmed (DKIM thread closed).** Follow-up on the 2026-05-07 DKIM cache issue: test mails from nest now pass at Gmail. SMTP relay through kaltenbach mailcow (`mail.kaltenbach.dev:587` STARTTLS, `nest@kaltenbach.dev`) → `hub/src/mail.js` → `POST /api/mail/test` button on `/appendages` all working. No fresh selector needed; the cache flush hypothesis from 2026-05-07 was right. Alert watchdog (`hub/src/alerts.js`) keeps using `sendMail()` for host/appendage state changes.
- 2026-05-21 — **Brownfield lifecycle + discovery backoff.** Closed the gap flagged on 2026-05-07 ("Lifecycle on adopted appendages is the next step"). New `hub/src/ssh-exec.js` extracts the spawn-with-timeout helper (`BatchMode=yes`, `ConnectTimeout=10`, hard SIGKILL on `timeoutMs`); used by both the poller and the new lifecycle routes. `ssh-discovery.js` now self-reschedules with **exponential backoff on poll failure** (60s → cap 600s, reset on success; logs first failure, every 5th, and recovery). Today's 02:56→03:02 stoneshop outage would have triggered ~3 polls + a recovery line instead of 6. New routes on `routes/appendages.js` (213 lines, slightly over the 200-line soft cap): `GET /api/appendages/:name/logs?container=&lines=` (clamped 1–2000, `--timestamps`), `GET /api/appendages/:name/inspect?container=`, `POST /api/appendages/:name/restart` (admin-only). `resolveBrownfield(name)` + `pickContainer(ctx, arg)` whitelist the `container` arg against the live containers matching the appendage's `match_any` patterns — exact-match, no substring, no shell-interpolation surface. **Smoke tests against live stoneshop:** mariadb logs returned 1121B of real output (mariadb writes to stderr); keydb inspect returned `state=running health=healthy`; `dockbase_keydb` restart via API succeeded (`durationMs:792`, container cycled `starting → healthy` in 12s, redis logs confirm clean RDB load); non-admin restart → 403; shell-injection attempt `"dockbase_keydb; rm -rf /"` → 400 rejected by whitelist; greenfield appendage restart → 400 "not a brownfield (discovery) appendage". **Not done yet:** UI hooks on `/appendages` (logs viewer, inspect modal, restart button); `update` action (each adopted stack uses different orchestration — defer until pattern is clear).
